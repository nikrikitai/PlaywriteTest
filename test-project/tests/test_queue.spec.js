...
test.describe('Тест сайта', () => {
     ...
     test('ID: RATE01 - Rate limiting на кнопке "Начать работу"', async ({ page }) => {
        ...
        await allure.step('1. Проверка данных окружения', async () => {
                ...
                await validateEnvVars(VALID_ENV_VARS_RATE, 'RATE01');
        ...
        let EmailField, PasswordField, Button, Button_test_page;
        ...
        await allure.step('5. Переход на тестируемую страницу', async () => {
            
            await page.goto(CONFIG.FULL_PAGE_URL_CONFIG);
        ...
        await allure.step('6. Вызываем ошибку и ждем восстановление после таймаута', async () => {
        ...
        await allure.step('7. Очистка cookies', async () => {
            await clearCookiesAndStorage(page);
        });

        await allure.step('8. Проверяем что разлогинились', async () => {
            await page.goto(CONFIG.FULL_LOGIN_URL_CONFIG);
            const elements = await verifyLoginForm(page);
            EmailField = elements.EmailField;
            PasswordField = elements.PasswordField;
